...
function site_health_check( $tests ) {
	$tests['direct']['pantheon_tls_check_failing_urls'] = [
		'label' => __( 'TLS Failing URLs', 'wp-tls-compatibility-checker' ),
...
function site_health_test() {
	...
	$result = [
		'status' => 'good',
		'label' => __( 'All scanned URLs pass TLS check', 'wp-tls-compatibility-checker' ),
		'description' => __( 'All scanned URLs support TLS 1.2 or higher.', 'wp-tls-compatibility-checker' ),
		'badge' => [
			'label' => __( 'TLS Compatibility', 'wp-tls-compatibility-checker' ),
			'color' => 'green',
		],
	...
	if ( ! empty( $failing_urls ) ) {
		...
		$result['label'] = __( 'TLS Failing URLs Detected', 'wp-tls-compatibility-checker' );
		$result['badge']['color'] = 'red';
		$result['description'] = sprintf(
			// Translators: %s is a list of URLs.
			__( 'The following URLs do not support TLS 1.2 or higher: %s', 'wp-tls-compatibility-checker' ),
